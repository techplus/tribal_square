! function ( a ) {
	...
	a.fn.doAutosize = function ( b ) {
		...
		return e = jQuery.extend( {
			...
			if ( "" != d && 1 != h ) {
				if ( a( "<span>" ).addClass( "tag" ).append( a( "<span>" ).text( d ).append( "&nbsp;&nbsp;" ) , a( "<a>" , {
						...
						title : "Removing tag" ,
						text : "x"
					} ).click( function () {return a( "#" + f ).removeTag( escape( d ) )} ) ).insertBefore( "#" + f + "_addTag" ), g.push( d ), a( "#" + f + "_tag" ).val( "" ), e.focus ? a( "#" + f + "_tag" ).focus() : a( "#" + f + "_tag" ).blur(), a.fn.tagsInput.updateTagsField( this , g ), e.callback && c[ f ] && c[ f ].onAddTag ) {
					var i = c[ f ].onAddTag;
					i.call( this , d )
				}
				if ( c[ f ] && c[ f ].onChange ) {
	...
	}, a.fn.importTags = function ( b ) {id = a( this ).attr( "id" ), a( "#" + id + "_tagsinput .tag" ).remove(), a.fn.tagsInput.importTags( this , b )}, a.fn.tagsInput = function ( e ) {
		var f = jQuery.extend( {
			interactive : ! 0 ,
		...
		return this.each( function () {
			...
			(! e || b[ a( this ).attr( "id" ) ]) && (e = a( this ).attr( "id" , "tags" + (new Date).getTime() ).attr( "id" ));
			var g = jQuery.extend( {
				pid           : e ,
				real_input    : "#" + e ,
				holder        : "#" + e + "_tagsinput" ,
			...
			b[ e ] = g.delimiter, (f.onAddTag || f.onRemoveTag || f.onChange) && (c[ e ] = new Array, c[ e ].onAddTag = f.onAddTag, c[ e ].onRemoveTag = f.onRemoveTag, c[ e ].onChange = f.onChange);
			var h = '<div id="' + e + '_tagsinput" class="tagsinput"><div id="' + e + '_addTag">';
			if ( f.interactive && (h = h + '<input id="' + e + '_tag" value="" data-default="' + f.defaultText + '" />'), h += '</div><div class="tags_clear"></div></div>', a( h ).insertAfter( this ), a( g.holder ).css( "width" , f.width ), a( g.holder ).css( "min-height" , f.height ), a( g.holder ).css( "height" , f.height ), "" != a( g.real_input ).val() && a.fn.tagsInput.importTags( a( g.real_input ) , a( g.real_input ).val() ), f.interactive ) {
				if ( a( g.fake_input ).val( a( g.fake_input ).attr( "data-default" ) ), a( g.fake_input ).css( "color" , f.placeholderColor ), a( g.fake_input ).resetAutosize( f ), a( g.holder ).bind( "click" , g , function ( b ) {a( b.data.fake_input ).focus()} ), a( g.fake_input ).bind( "focus" , g , function ( b ) {a( b.data.fake_input ).val() == a( b.data.fake_input ).attr( "data-default" ) && a( b.data.fake_input ).val( "" ), a( b.data.fake_input ).css( "color" , "#000000" )} ), void 0 != f.autocomplete_url ) {
					...
					void 0 !== jQuery.Autocompleter ? (a( g.fake_input ).autocomplete( f.autocomplete_url , f.autocomplete ), a( g.fake_input ).bind( "result" , g , function ( b , c ) {
						c && a( "#" + e ).addTag( c[ 0 ] + "" , {
							focus : ! 0 ,
							unique : f.unique
						} )
					} )) : void 0 !== jQuery.ui.autocomplete && (a( g.fake_input ).autocomplete( autocomplete_options ), a( g.fake_input ).bind( "autocompleteselect" , g , function ( b , c ) {
						return a( b.data.real_input ).addTag( c.item.value , {
							focus : ! 0 ,
							unique : f.unique
						} ), ! 1
					} ))
				} else a( g.fake_input ).bind( "blur" , g , function ( b ) {
				...
				a( g.fake_input ).bind( "keypress" , g , function ( b ) {
				...
				} ), g.removeWithBackspace && a( g.fake_input ).bind( "keydown" , function ( b ) {
				...
				} ), a( g.fake_input ).blur(), g.unique && a( g.fake_input ).keydown( function ( b ) {(8 == b.keyCode || String.fromCharCode( b.which ).match( /\w+|[áéíóúÁÉÍÓÚñÑ,/]+/ )) && a( this ).removeClass( "not_valid" )} )
			}
		} ), this
	...
	var d = function ( b ) {
		var c = ! 1;
		return 13 == b.which && (c = ! 0), a.each( b.data.delimiter , function ( a , d ) {b.which == d.charCodeAt( 0 ) && (c = ! 0)} ), c
	}
}( jQuery );